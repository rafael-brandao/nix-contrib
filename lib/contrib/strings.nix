...
in rec {
  ...
  */
  ...
  }: list: "${start}${concatMapStringsSep sep toString list}${end}";
  ...
  */
  trim = string: let
    ...
    calculateIndexes = left: right:
      ...
      in
        if (isNotBlankChar leftChar) && (isNotBlankChar rightChar)
        then {
          start = left;
          len = right - left + 1;
        }
        else if (isBlankChar leftChar) && (isBlankChar rightChar)
        then calculateIndexes (left + 1) (right - 1)
        else if (isNotBlankChar leftChar) && (isBlankChar rightChar)
        then calculateIndexes left (right - 1)
