{lib}: let
  ...
  testEmptyness = functionName: acceptedTypes: testFunction: let
    requireType = x: let
      type = typeOf x;
      isAcceptedType = any (t: t == type) acceptedTypes;
      errorMsg = "${functionName}: invalid argument: ${toString x} :: ${escapeNixString type}; accepted type is one of: ${concatStringsSep ", " (map escapeNixString acceptedTypes)}";
    in
